...
//            ResultSet rs = st.executeQuery("select * from imagepoint where accountno='" + accountno + "'");
            String key="secure";
             ResultSet rs = st.executeQuery("select AES_DECRYPT(x,'"+key+"'),AES_DECRYPT(y,'"+key+"') from imagepoint where accountno='" + accountno + "'");
            if (rs.next()) {
                ...
                if ((x1<=xpoint1 && x1>=xpoint2) && (y1<=ypoint1 && y1>=ypoint2)) {
                    RequestDispatcher rd = request.getRequestDispatcher("login_choosepoint1.jsp");
