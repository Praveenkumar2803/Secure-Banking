...
public class action extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String x=request.getParameter("images.x");
         ...
         String cnt=(String)hs.getAttribute("cnt");
         try
         {
             Class.forName("com.mysql.jdbc.Driver");
                ...
                st.executeUpdate("insert into imagepoint values ('"+accountno+"',AES_ENCRYPT('"+x+"','"+key+"'),AES_ENCRYPT('"+y+"','"+key+"'),'1')");  
                System.out.println("The account no is " +accountno);
                ResultSet rs=st.executeQuery("select * from imagepoint where accountno='"+accountno+"' and cnt='1'" );
                if(rs.next())
                {
...
//                RequestDispatcher rd=request.getRequestDispatcher("choosepointok.jsp");
                   RequestDispatcher rd=request.getRequestDispatcher("choosepoint1.jsp");
