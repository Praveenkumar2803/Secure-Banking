        ...
        String x=request.getParameter("images.x");
         ...
         try
         {
             Class.forName("com.mysql.jdbc.Driver");
                ...
                String key="secure";
//                st.executeUpdate("insert into imagepoint1 values('"+accountno+"','"+x+"','"+y+"','2')");  
                st.executeUpdate("insert into imagepoint1 values ('"+accountno+"',AES_ENCRYPT('"+x+"','"+key+"'),AES_ENCRYPT('"+y+"','"+key+"'),'2')");  
                System.out.println("The account no is " +accountno);
               ResultSet rs=st.executeQuery("select * from imagepoint1 where accountno='"+accountno+"' and cnt='2'" );
                if(rs.next())
                {
                    System.out.println("Next page is...");
                    request.setAttribute("cnt", 2);
                RequestDispatcher rd=request.getRequestDispatcher("Loginphoto.jsp");
